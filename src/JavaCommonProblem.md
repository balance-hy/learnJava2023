...
> 在Java中，`hashCode()`和`equals()`两个方法是Object类中的方法，通常需要一起重写。如果只重写其中一个，可能会导致一些问题。
>
> 1. **只重写`equals()`**：如果只重写了`equals()`方法，那么当你把这个类的对象添加到哈希表（如HashSet、HashMap）中时，可能会出现问题。因为哈希表是通过`hashCode()`来确定对象的存储位置的，如果你没有重写`hashCode()`，那么默认的`hashCode()`方法可能会为不同的对象产生相同的哈希码，这样就会导致哈希表的性能下降。另外，如果两个对象是相等的（即`equals()`返回true），但它们的哈希码不相等，那么哈希表可能无法正确地找到这两个对象。
> 2. **只重写`hashCode()`**：如果只重写了`hashCode()`方法，那么可能会导致两个本应相等的对象被判断为不相等。因为在Java中，我们通常认为如果两个对象是相等的（即`equals()`返回true），那么它们的哈希码也应该相等。如果你没有重写`equals()`，那么默认的`equals()`方法会比较对象的内存地址，这样就可能会导致两个本应相等的对象被判断为不相等。对象的内容相等就应该判断为相等对象
>
> 因此，当你需要重写`equals()`或`hashCode()`时，通常应该同时重写这两个方法，以保证它们的行为一致。
